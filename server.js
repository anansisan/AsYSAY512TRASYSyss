...
setInterval(() => {
  http.get(`http://.glitch.me/megasyst`);
}, 280000);
  ...
  client.on('message', msg => {  
   ...
   client.on('message', msg => {  
    if (msg.content === 'سلام عليكم') {  
      msg.reply('**وعليكم السلام ورحمه الله و بركاته :heart: **');  
    }
  });

  client.on('message', msg => {  
    if (msg.content === 'Rooney') {  
      msg.reply('** معاك :heart: **');  
    }
  });


  client.on('message', msg => {  
    if (msg.content === '@Mega 𝐒𝐲𝐬𝐭𝐞𝐦#6561 ') {  
      msg.reply('** ال بريفكس - :heart: **');  
    }
  });
...
console.log("Starting..");724
let g = client.guilds.get("769647701750947"); // 769647701750947  
let c = g.channels.get("769647701750972452");// 769647701750972452
if(c.type === 'voice') {
c.join();
setInterval(() => {
...
let g = client.guilds.get("785975818844504064"); // id server
let c = g.channels.get("786461902652833803");// id channel
if(c.type === 'voice') {
c.join();
setInterval(() => {
if(g.me.voiceChannel && g.me.voiceChannelID !== c.id || !g.me.voiceChannel) c.join();
...
client.login('Nzg2NDY0MzI0NjI5NTYxMzg2.X9Gx-g.yRlI101pSYE67ckFnTDMVNiytas');
